...
if __name__ == '__main__':
    ...
    # data: 指向刚才写的 yaml 文件
    results = model.train(
        data=r'C:\Users\ASUS\Desktop\yolo_ele\2026_yolov11_electric\01_Dataset\data.yaml',  # 改成你的绝对路径

        # --- 基础配置 ---
        imgsz=640,  # 【图像尺寸统一化】自动将所有图缩放/填充到 640x640
        epochs=230,  # 训练轮数，建议 100-300
        batch=32,  # 4060 8G显存可以开 16 或 32，如果爆显存就改小
        device=0,  # 指定使用显卡
        workers=4,  # 数据加载线程数
        ...
        hsv_s=0.6,  # 【色彩调整】饱和度随机变化
        hsv_v=0.3,  # 【色彩调整】亮度随机变化
        mosaic=1.0,  # 马赛克增强 (YOLO核心增强，拼接4张图，强烈建议开启)

        # --- 结果保存 ---
        project='runs/train',  # 训练结果保存在哪里
        name='exp_4090_v1_m',  # 实验名称
    )